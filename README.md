...
Webfont tags load user-supplied webfont resources through inline `@font-face` rules, preconnect hints, or font preloads. The extension does not apply `font-family`; choose the loaded family in `term.ptt.cc` settings. Remote stylesheet links, script, iframe, image, arbitrary HTML tags, and inline style rules outside `@font-face` are rejected.
